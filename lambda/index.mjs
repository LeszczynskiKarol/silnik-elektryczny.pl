...
function isMotor(product) {
  ...
  if (cats.some((c) => EXCLUDED_SLUGS.has(c.slug || c.category?.slug || "")))
    return false;
  return cats.some((c) => {
    ...
    if (productMatch && method === "GET") {
      ...
      if (!data.success || !isEligible(data.data.product))
        return {
          statusCode: 404,
          headers: CORS,
          body: JSON.stringify({ success: false, error: "Nie znaleziono" }),
        };
      return {
        statusCode: 200,
    ...
    if (path === "/create-checkout" && method === "POST") {
      ...
      if (!items?.length || !shipping?.email) {
        return {
          statusCode: 400,
          headers: CORS,
          body: JSON.stringify({ success: false, error: "Brak danych" }),
        };
      }
      ...
      if (paymentMethod === "cod" && totalWeight > 575) {
        return {
          ...
          body: JSON.stringify({
            success: false,
            error: "Za pobraniem niedostępne dla wagi powyżej 575 kg",
          }),
        };
      }
